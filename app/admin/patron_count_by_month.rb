ActiveAdmin.register_page "Count By Month" do
  ...
  content do
    ...
    within @head do
          script :src => javascript_path('patron_counts_by_month.js'), :type => "text/javascript"
    end
    render 'admin/data/patron_count_by_month_charts', context: self, locals: {opts: @arbre_context.assigns[:opts]}
  end #content


  sidebar 'Patron Count By Year' do
    para "Patron counts for each branch by month are represented in each graph."

    para "The solid line represents the average patron count, the dots represent the maximum count for the month. "
  ...
  sidebar :filters, partial: 'admin/data/patron_count_by_month_filters', context: self
#
  controller do
    def index
